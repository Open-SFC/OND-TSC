                                   ...
                                   uint8_t pkt_origin)
{
      ...
      if(nsc_nw_function_found == 1) /* No more network functions present in the nschain. */
      {
        ...
        do
        {
          ...
          if(retval == OF_SUCCESS)
          {
            ...
            nschain_repository_entry_p->tun_dest_ip = dst_crm_port_p->switch_ip;
            nschain_repository_entry_p->port_type   = dst_crm_port_p->crm_port_type;
            nschain_repository_entry_p->nw_port_b   = FALSE;
            //printf("\r\n nsmrmip  = %x",dst_crm_port_p->switch_ip);
            //printf("\r\n nsmlport = %x",dst_crm_port_p->port_id);         
          } 
          else
          {
            retval = OF_FAILURE;
          ...
          if((nschain_repository_entry_p->port_type == VM_GUEST_PORT)
                                      ||
            ...
            )
          {
              ...
              //printf("\r\n nsmnwport = %x",nschain_repository_entry_p->out_port_no);
              nschain_repository_entry_p->ns_port_b  =FALSE;
  ...
  if(nschain_repository_entry_p->more_nf_b == TRUE)
  {
    ...
    if(strcmp(nschain_repository_entry_p->local_switch_name_p,nwservice_instance_p->switch_name))   
    {
      ...
      nschain_repository_entry_p->vm_and_service1_not_on_same_switch = 1; /* Proactive pushing */
...
int32_t tsc_send_all_flows_to_all_tsas(struct nschain_repository_entry* nschain_repository_entry_p)
{
  ...
  if(nschain_repository_entry_p->skip_this_entry == 1)
  {
    CNTLR_RCU_READ_LOCK_RELEASE(); 
    printf("\r\n PPH-50"); 
    return OF_SUCCESS;
  }
  /* This function shall be called only with that */
  if((nschain_repository_entry_p->selector.selector_type != SELECTOR_PRIMARY)) 
  {
    OF_LOG_MSG(OF_LOG_TSC,OF_LOG_ERROR,"Selector is not Primary.Return without pushing all the flows for the traffic");
    CNTLR_RCU_READ_LOCK_RELEASE();
    printf("\r\n PPH-51");
    return OF_SUCCESS;
  }    
  ...
  if(nschain_repository_entry_p->vm_and_service1_not_on_same_switch)
  {
      printf("NOT-SSW");
...
int32_t tsc_nsc_send_proactive_flow(struct    nschain_repository_entry* nschain_repository_entry_first_p,
                                    ...
                                    struct    nschain_repository_entry** repository_entry_1_p)   
{    
  uint8_t  nw_type,heap_b;
  uint8_t  status = FALSE;
  uint32_t src_ip,dst_ip,nid,hashkey,hashmask,offset,index,magic,ii;
  uint16_t src_port,dst_port,icmp_type,icmp_code;
  ...
  {
    src_ip   = nschain_repository_entry_first_p->selector.src_ip;
    dst_ip   = nschain_repository_entry_first_p->selector.dst_ip;

    if(nschain_repository_entry_first_p->selector.protocol == OF_IPPROTO_ICMP)
    {
      icmp_type = 8;
      icmp_code = 0;
      src_port  = 8;
    ...
    {
      src_port = nschain_repository_entry_first_p->selector.src_port;
      dst_port = nschain_repository_entry_first_p->selector.dst_port; 
    }  
  }    
  else if (tsc_flow_p->selector_type == SELECTOR_SECONDARY)
  {
    src_ip   = nschain_repository_entry_first_p->selector.dst_ip;
    dst_ip   = nschain_repository_entry_first_p->selector.src_ip;
    ...
    {     
      src_port = nschain_repository_entry_first_p->selector.dst_port;
      dst_port = nschain_repository_entry_first_p->selector.src_port;
    }  
  }
  ...
  hashkey = nsc_compute_hash_repository_table_1_2(src_ip,
                                                  dst_ip,
                                                  src_port,
                                                  dst_port,
                                                  nschain_repository_entry_first_p->selector.protocol,
                                                  nschain_repository_entry_first_p->selector.ethernet_type,
  ...
  MCHASH_BUCKET_SCAN(nsc_repository_table_p,hashkey,nsc_repository_scan_node_p,struct nschain_repository_entry *, offset)
  {
    ...
    if((src_ip   != nsc_repository_scan_node_p->selector.src_ip )  ||
       (dst_ip   != nsc_repository_scan_node_p->selector.dst_ip )  ||
       (nschain_repository_entry_first_p->selector.protocol != nsc_repository_scan_node_p->selector.protocol) ||
       (src_port != nsc_repository_scan_node_p->selector.src_port) ||
       (dst_port != nsc_repository_scan_node_p->selector.dst_port)
                                    ||
       (nschain_repository_entry_first_p->selector.ethernet_type != nsc_repository_scan_node_p->selector.ethernet_type)
                                    ||
  ...
  nsc_repository_entry_p->selector = (nschain_repository_entry_first_p->selector); /* to copy original mac addresses etc */
  ...
  OF_LOG_MSG(OF_LOG_TSC, OF_LOG_DEBUG,"pkt_origin = %d",tsc_flow_p->pkt_origin);

  nsc_repository_entry_p->nw_type    = nw_type;
  OF_LOG_MSG(OF_LOG_TSC, OF_LOG_DEBUG,"nw_type    = %d",nsc_repository_entry_p->nw_type);
  ...
  nsc_repository_entry_p->selector.icmp_type = icmp_type; 
  nsc_repository_entry_p->selector.icmp_code = icmp_code; 
  nsc_repository_entry_p->selector.src_port  = src_port;  
  nsc_repository_entry_p->selector.dst_port  = dst_port;  
  nsc_repository_entry_p->selector.src_ip    = src_ip;    
  nsc_repository_entry_p->selector.dst_ip    = dst_ip;    
  ...
  else
  {
    ...
    do
    {
      ...
      if(retval == OF_SUCCESS)
      {
        ...
        nsc_repository_entry_p->tun_dest_ip = dst_crm_port_p->switch_ip;
        /* Used if it is a remote port */
        OF_LOG_MSG(OF_LOG_TSC, OF_LOG_DEBUG,"tun_dest_ip = %x",dst_crm_port_p ->switch_ip);
        nsc_repository_entry_p->port_type = dst_crm_port_p->crm_port_type;
      ...
      if((nsc_repository_entry_p->port_type == VM_GUEST_PORT)
                                   ||
                                            ...
                                            uint8_t   use_dstmac )
{
  ...
  strcpy(ucastpkt_outport_repository_entry_p->local_switch_name_p,nschain_repository_entry_first_p->local_switch_name_p);
